...
pygame_font = 'Comic Sans MS'

# The background image file name to use
background_image_location = 'background_10.png'
# The rover image file location to use
rover_image_location = 'rover_6.png'
...
class RoverDomainVisualizer(object):
    ...
    def __init__(self, rover_count, grid_size, resolution=(window_width, window_height),
                 window_offset=(window_width_offset, window_height_offset)):
    ...
    def update_visualizer(self, rover_pos_list, poi_pos_list, poi_status_list, reset=False, wait_time=0.1):
            ...
            for rov_id in range(len(adjusted_rover_pos_list)):
                ...
                # In case the time step is an earlier one
                    ...
                    pygame.draw.circle(self.pygame_display, rgb_color,
                                       (adjusted_rover_pos[0],
                                        adjusted_rover_pos[1]),
                                       self.circle_radius)
                    ...
                    pygame.draw.line(self.pygame_display, rgb_color,
                                     (adjusted_rover_pos[0],
                                      adjusted_rover_pos[1]),
                                     (next_step_adjusted_rover_pos[0],
                                      next_step_adjusted_rover_pos[1]),
                                     self.line_width)
...
if __name__ == "__main__":
    ...
    visualizer = RoverDomainVisualizer(rover_count, grid_size)
    ...
    for i in range(rounds_to_run):
        ...
        if i % reset_interval == 0:
            ...
            # Generating a POI status list
            poi_status_list = [False for _ in range(poi_count)]
        ...
        visualizer.update_visualizer(rover_coordinate_list, poi_coordinate_list, poi_status_list, reset, waiting_time)
        ...
        # We check whether to randomly make a certain POI observed
        if poi_prob > random.random():
            # We update the poi status
            poi_status_list[random.randint(0, poi_count-1)] = True
