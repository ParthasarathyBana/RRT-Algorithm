...
def find_path(map_matrix, start, goal, rrt_growth_limit, terminal_goal_distance,
    ...
    # The path length in the (number of node in path - 1) * rrt growth limit
    rrt_path_length = (rrt_path_nodes_count-1)*rrt_growth_limit
    # We measure the time to perform the path planning
    end_time = time.time()
    ...
    return nodes_in_path[:-1], rrt_path_length, end_time-start_time, nodes_in_branches
